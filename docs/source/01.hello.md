...
第 1 行 `// SPDX-License-Identifier: MIT` 就是合约的版权声明。其中 **SPDX-License-Identifier** (SPDX 许可标示) 是标注当前的智能合约采用什么样的对外开放标准，该标准规定了别人是否拥有商业开发，学习使用等权利。代码中使用的 `MIT` 规定了其他人随便用该代码，但出问题不负责。
...
更多的 SPDX-License-Identifier 类型介绍，参照文章 SPDX License List(https://spdx.org/licenses/) 详细阅读。
...
- **扩展:**
  1. Solidity 遵循 [npm 的 license 建议](https://docs.npmjs.com/cli/v7/configuring-npm/package-json#license)
  2. 更多的 bytecode metadata 参照文章 [合约的 metadata](#)
